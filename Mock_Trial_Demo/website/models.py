...
class UserForm(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
    education = db.Column(db.String(500*15))
    skills = db.Column(db.String(500*15))
    experiences = db.Column(db.String(500*15))

class UserCV(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
    details = db.Column(db.String(500*50))  # Store the extracted CV details here
