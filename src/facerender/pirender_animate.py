...
class AnimateFromCoeff_PIRender():
    ...
    def generate(self, x, video_save_dir, pic_path, crop_info, enhancer=None, background_enhancer=None, preprocess='crop', img_size=256):
        ...
        # frames_dir = os.path.join(video_save_dir, x['video_name'])
        ...
        with torch.no_grad():
            ...
            for i in tqdm(range(target_semantics.shape[1]), 'FaceRender:'):
                generated_frame = self.net_G(source_image, target_semantics[:, i])['fake_image']
                predictions_video.append(generated_frame)
                # print("!!!!!! ", len(predictions_video))

                # for j in range(generated_frame.shape[0]):  # j will be 0 or 1
                #     # Calculate the filename based on i and j
                #     if j == 0:
                #         frame_num = i
                #     else:
                #         frame_num = i + total_frames

                #     frame_image = generated_frame[j].data.cpu().numpy()
                #     frame_image = np.transpose(frame_image, [1, 2, 0]).astype(np.float32)
                #     frame_image = img_as_ubyte(frame_image)

                #     frame_path = os.path.join(frames_dir, f'frame_{frame_num:04d}.png')
                #     cv2.imwrite(frame_path, cv2.cvtColor(frame_image, cv2.COLOR_RGB2BGR))

                # Save the generated frame
                # if generated_frame.dim() == 4 and generated_frame.shape[0] == 1:
                #     generated_frame = generated_frame.squeeze(0)
                # print(generated_frame.shape)
                # frame_image = np.transpose(generated_frame.data.cpu().numpy(), [1, 2, 0]).astype(np.float32)
                # frame_image = img_as_ubyte(frame_image)
        ...
        # frames = frame_num
        frames = len(predictions_video) - 1 
